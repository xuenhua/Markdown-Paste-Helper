/**
 * Xiaohongshu (小红书) platform handler
 *
 * Strategy: convert-to-html mode with automated image insertion.
 * 小红书 uses a Tiptap/ProseMirror editor.
 * Selector: .tiptap.ProseMirror[contenteditable="true"]
 *
 * Limitations: No native table or code block support.
 * Tables: Converted to text-based code blocks in Markdown (rendered as plain text).
 * Images: Replaced with markers, then postPasteCleanup downloads and pastes them.
 */
(function () {
  "use strict";
  console.log(
    "%c[Markdown Paste Helper] ✅ xiaohongshu.js 已加载",
  ...
  function preprocessMarkdown(text) {
    ...
    // Markdown tables → text code blocks (小红书 doesn't support tables)
    const tableRe = /((?:^|\n)\|.+\|\n\|[\s:|-]+\|\n(?:\|.+\|\n?)+)/g;
    ...
    // Convert task list checkboxes to emoji
  ...
  async function postPasteCleanup() {
    ...
    console.log(`[小红书 Auto] 按文档顺序处理 ${items.length} 张图片...`);
    ...
    const downloadTasks = items.map(async (it) => {
      try {
        ...
        console.log(`[小红书 Auto] ✅ 预下载完成: ${it.alt || it.url}`);
      } catch (err) {
        console.warn(
          `[小红书 Auto] ❌ 预下载失败: ${it.alt} — ${err.message}`,
        );
        it._blob = null;
      }
    });
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        ...
        if (!markerInfo) {
          console.warn(`[小红书 Auto] ❌ 未找到: ${item.marker}`);
          continue;
        }
        ...
        console.log(`[小红书 Auto] ✅ 图片插入完成`);
        await sleep(1500);
      } catch (err) {
        console.error(`[小红书 Auto] 插入图片失败:`, err);
      }
    }
    ...
    console.log("[小红书 Auto] ✅ 图片全部插入完成");

    if (removeToast && progressToast) removeToast(progressToast);
    if (showToast) showToast("✅ 小红书 图片全部上传并插入完成！", "success");
  }
      ...
      const isAdjacentToImage =
        (prev &&
          (prev.tagName === "FIGURE" ||
            ...
            prev.getAttribute("data-dom-type") === "image")) ||
        (next &&
          (next.tagName === "FIGURE" ||
            next.querySelector("img, figure") ||
            next.getAttribute("data-dom-type") === "image"));
  ...
  window.PlatformHandlers.xiaohongshu = {
