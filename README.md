...
| **51CTO**        | HTML + 图片上传 | am-engine 编辑器，图片自动上传                  |
| **慕课网**       | 预处理文本      | cledit (StackEdit) 原生 Markdown 编辑器         |
| **小红书**       | HTML + 图片上传 | Tiptap/ProseMirror，表格转文本代码块            |
...
│       ├── 51cto.js           # 51CTO 处理器（图片自动上传）
│       ├── imooc.js           # 慕课网处理器
│       └── xiaohongshu.js     # 小红书处理器（图片自动上传）
├── popup/
│   ├── popup.html             # 弹窗 UI
│   └── popup.js               # 弹窗逻辑 + 公众号设置
├── lib/                       # 第三方库
...
| 51CTO        | am-engine (AOMAO)     | `convert-to-html` + 图片异步上传           |
| 慕课网       | cledit (StackEdit)    | `preprocess-text`：原生 Markdown           |
| 小红书       | Tiptap/ProseMirror    | `convert-to-html` + 图片异步上传           |

### 图片自动上传机制

对于 B站、头条号、51CTO 等平台，插件采用：
